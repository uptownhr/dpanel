...
dpanel.init().then(function () {
  ...
  program.command('start <domain> [image]').description('start a vhost with image').option('-i, --image [image]', 'Specify [image] to create container with').option('-v [volume]', '[volume] mount to container').option('-p [container port]', '[port] container is listening on').action(function (domain, image, options) {
    var port = options.P;
    var volume = options.V;

    if (!image) {
      dpanel.docker.containerExistsByName(domain).then(function (container) {
        dpanel.start(domain, container.Image).then(function (container) {
        ...
        prompt.get({
          ...
          dpanel.start(domain, image, port, volume).then(function (container) {
            console.log('started', container);
          }).fail(console.log);
        });
      });
  ...
  program.command('stop <domain>').description('stop a site').action(function (domain) {
    dpanel.stop(domain).then(console.log, console.log);
  });

  program.command('delete <domain>').description('delete a site').action(function (domain) {
    dpanel['delete'](domain).then(console.log, console.log);
  });

  program.command('keys <action>').description("generate - creates ssh keys\n   copy - copies pubkey to a container of choice").action(function (action) {
    switch (action) {
      case "generate":
  ...
  program.command('list').description('list available sites').action(function () {
    dpanel.list().then(function (containers) {
      var table = new Table({
        head: ['Domain'.cyan, 'Image'.cyan, 'Uptime'.cyan]
      });
      containers.forEach(function (container) {
        table.push([container.Names[0].substring(1), container.Image, container.Status]);
